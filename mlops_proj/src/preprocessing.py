...
class TMDB_preprocessor:
    def __init__(
        ...
        random.seed(0) # fix seed for reproductivity
        self._movies = movies
    ...
    def generate_watch_second(self, rating: float) -> int:
        base = 1.1
        noise_level = 0.1
    ...
    def selection(self, user_id: int, features) -> list:
        select_count = random.randint(0, self._max_select_count)
        print(f"user[{user_id}] selects [{select_count}] contents")
